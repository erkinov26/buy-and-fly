...
export class GoogleAnalytics {
  ...
  sendNavigation(url: string): void {
    if (
      (this.isBrowser &&
        !this.config.domains?.every(
          (domain) => this.document.referrer.indexOf(domain) < 0,
        )) ||
      !this.config.paths?.every(
        (path) => this.document.location.pathname.indexOf(path) < 0,
      )
    ) {
      this.set({
        page_referrer: this.document.defaultView?.location.origin ?? '',
      });
    }
